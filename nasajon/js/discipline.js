...
$('#disciplineFgAtv').change(function (e) { 
    e.preventDefault();
...
function searchDiscipline() {
    ...
    objFilters.disciplineShdesc = $("#disciplineFullName").val();		
    ...
    tblDisciplineSearch.table().clear();
        
    getDisciplineByFilterTable(tblDisciplineSearch, objFilters, null, null);	
}


// console.log(arrayDate);
tblDisciplineSearch = $('#disciplineTable').DataTable({
    columnDefs: [ {
        orderable: false,
...
$('#disciplineTable tbody').on( 'click', 'tr', function () {
    if ( $(this).hasClass('selected') ) {
        $(this).removeClass('selected');
    }
    else {
        tblDisciplineSearch.$('tr.selected').removeClass('selected');
        $(this).addClass('selected');
    };
    //console.log('flgLocked -> ' + tblDisciplineSearch.row($("#" + tblDisciplineSearch.table().node().id + " tr.selected")).data().flgLocked);
    if (tblDisciplineSearch.rows('.selected').any() /*&& $('#btnSearchAction').hasClass('disabled')*/) {			
        $('#divSearchActionOptions').html(divSearchActionOptionsParm);	  
    
            let rowsData = tblDisciplineSearch.rows($("#" + tblDisciplineSearch.table().node().id + " tr.selected")).data();
            objDelete = rowsData[0].shdesc;
            $('.dropdown-item').click(function () { 
                tableActions(tblDisciplineSearch, this);  
            });
           
        $('#btnSearchAction').removeClass('disabled btn-default');
        $('#btnSearchAction').addClass('btn-info');
    } else if (!tblDisciplineSearch.rows('.selected').any() && !$('#btnSearchAction').hasClass('disabled')) {				
        $('#btnSearchAction').removeClass('btn-info');
        $('#btnSearchAction').addClass('disabled btn-default');
    };
} );

$('#disciplineTable tbody').on( 'dblclick', 'tr', function () {
