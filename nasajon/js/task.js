...
$('#taskID, #taskFullName,#taskFgAtv').keypress(function(event) {
...
$('.new-button').click(function (e) { 
    location.href = 'taskEdit';
    
});

function searchTask() {
    var objFilters = new Object();
    objFilters.taskID = $("#taskID").val();
    objFilters.shdesc = $("#taskFullName").val();		
    objFilters.fgAtv = $("#taskFgAtv").val();		
    ...
    tblTaskSearch.table().clear();
        
    getTaskByFilterTable(tblTaskSearch, objFilters, null, null);	
}


// console.log(arrayDate);
tblTaskSearch = $('#taskTable').DataTable({
    columnDefs: [ {
        orderable: false,
...
$('#taskTable tbody').on( 'click', 'tr', function () {
    if ( $(this).hasClass('selected') ) {
        ...
        tblTaskSearch.$('tr.selected').removeClass('selected');
        $(this).addClass('selected');
    };

    if (tblTaskSearch.rows('.selected').any() /*&& $('#btnSearchAction').hasClass('disabled')*/) {			
        $('#divSearchActionOptions').html(divSearchActionOptionsParm);	  
    
            let rowsData = tblTaskSearch.rows($("#" + tblTaskSearch.table().node().id + " tr.selected")).data();
            objDelete = rowsData[0].shdesc;
            $('.dropdown-item').click(function () { 
                tableActions(tblTaskSearch, this);  
            });
           
        $('#btnSearchAction').removeClass('disabled btn-default');
        $('#btnSearchAction').addClass('btn-info');
    } else if (!tblTaskSearch.rows('.selected').any() && !$('#btnSearchAction').hasClass('disabled')) {				
        $('#btnSearchAction').removeClass('btn-info');
        $('#btnSearchAction').addClass('disabled btn-default');
    };
} );
