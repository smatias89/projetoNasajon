function tableActionsCalendar(objDataTable, objAction,calendar){
    ...
    var rowsDataID = rowsData[0].scaleID
    ...
    if (objAction.id == 'searchEventTable') {
        ...
        // console.log(event);
        // console.log('rowsDataID: ',rowsDataID);
        ...
        if (eventSearch) {
            calendar.gotoDate(eventSearch.start); //start = start
...
function initCalendarMobile() {
    ...
    tblLoaderEventSearch = $('#tableLoaderEventSearch').DataTable({
        ...
        columnDefs: [ 
            {
                ...
                targets:   0,
                data: null
            }, { targets: [0, 1, 4], visible: false } // hide column
            // {
            //     orderable: false,
            //     // className: 'select-checkbox',
            //     targets:   5,
        ...
        rowId: 'scaleID',
        dom: 
            "<'row'<'col-sm-2'l><'col-sm-3'B><'col-sm-1 text-center divDropdownEvent btn-group dropdown'><'offset-sm-1 col-sm-2'><'col-sm-3'f>>" +
            "<'row'<'col-sm-12'tr>>" +
    ...
    $('#tableLoaderEventSearch tbody').on( 'click', 'tr', function () {
        ...
        if (tblLoaderEventSearch.rows('.selected').any() /*&& $('#btnSearchAction').hasClass('disabled')*/) {			
            ...
            $('.dropdown-item').click(function () {
                tableActionsCalendar(tblLoaderEventSearch, this,calendar);
            });
    ...
    $('#searchCalendar').click(function (e) { 
        e.preventDefault();
        console.log('Search Event Calendar modal...');

        var objFilters = new Object();
        objFilters.shdesc = $('#searchEventName').val();
        getCalendarSearchByFilter(objFilters,tblLoaderEventSearch)
        return

    });

    getCalendarLoaderEventMobile(calendar, {})
}
...
// console.log('date'+ new Date(y, m, d + 1, 19, 0));
calendar.render();
...
$("#calendar").on("click", ".editEvent", function (e) {
    ...
    if (idTmp != null || idTmp != '') {
        ...
        console.log('consulta... getCalendarByFilterEventType');
        // getCalendarByFilterEventType(objFilters)
