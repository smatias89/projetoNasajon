...
$('#scaleTypeID, #scaleTypeFullName,#scaleTypeFgAtv').keypress(function(event) {
...
$('#scaleTypeFgAtv').change(function (e) { 
    e.preventDefault();
...
$('.new-button').click(function (e) { 
    location.href = 'scaleTypeEdit';
    
});

function searchScaleType() {
    var objFilters = new Object();
    objFilters.scaleTypeID = $("#scaleTypeID").val();
    objFilters.shdesc = $("#scaleTypeFullName").val();		
    objFilters.fgAtv = $("#scaleTypeFgAtv").val();		
    ...
    tblScaleTypeSearch.table().clear();
        
    getScaleTypeByFilterTable(tblScaleTypeSearch, objFilters, null, null);	
}


// console.log(arrayDate);
tblScaleTypeSearch = $('#scaleTypeTable').DataTable({
    columnDefs: [ {
        orderable: false,
...
$('#scaleTypeTable tbody').on( 'click', 'tr', function () {
    if ( $(this).hasClass('selected') ) {
        $(this).removeClass('selected');
    }
    else {
        tblScaleTypeSearch.$('tr.selected').removeClass('selected');
        $(this).addClass('selected');
    };

    if (tblScaleTypeSearch.rows('.selected').any() /*&& $('#btnSearchAction').hasClass('disabled')*/) {			
        $('#divSearchActionOptions').html(divSearchActionOptionsParm);	  
    
            let rowsData = tblScaleTypeSearch.rows($("#" + tblScaleTypeSearch.table().node().id + " tr.selected")).data();
            objDelete = rowsData[0].shdesc;
            $('.dropdown-item').click(function () { 
                tableActions(tblScaleTypeSearch, this);  
            });
           
        $('#btnSearchAction').removeClass('disabled btn-default');
        $('#btnSearchAction').addClass('btn-info');
    } else if (!tblScaleTypeSearch.rows('.selected').any() && !$('#btnSearchAction').hasClass('disabled')) {				
        $('#btnSearchAction').removeClass('btn-info');
        $('#btnSearchAction').addClass('disabled btn-default');
    };
} );
...
$('.deleteConfirmation').click(function () {
    deleteScaleType($('.search-button'),objDelete);		
    
});

$('#scaleTypeSearch').click();
